          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center justify-between">
               <div className="text-sm text-blue-800">
                 ...
                 Drivers (from drivers collection): {profiles.filter(p => p.source === 'driver').length} | 
                 Admins/Owners (from profiles collection): {profiles.filter(p => p.source === 'profile').length} | 
            ...
            {profiles.length > 0 && (
              <div className="text-xs text-blue-600 mt-2 p-2 bg-blue-100 rounded break-words">
                <strong>Users shown:</strong> {profiles.map(p => `${p.name} (${p.email}, ${p.source})`).join(' • ')}
              </div>
            )}
